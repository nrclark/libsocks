                    ...
                    teardown_t teardown)
{
    ...
    suites = realloc(suites, (suite_count + 1) * sizeof(full_suite_t *));

    suites[suite_count] = malloc(sizeof(full_suite_t) + name_length + 1);
...
static int suite_runner(test_t *suite, char *name, setup_t setup,
                        teardown_t teardown)
{
...
int main(void)
{
    ...
    printf("Test suites run: %d. (suites passed: %d, suites failed: %d)\n",
